...
// PublishWaitAsync  executes callback defined for a topic. Any additional argument will be transferred to the callback.
		...
		handler.once.Do(func() {
			...
			for idx, h := range bus.handlers[topic] {
				...
				if h.once == handler.once {
					err := bus.removeHandler(topic, idx)
					if err != nil {
						// Log error here
					}
					break
...
func (bus *EventBus) removeHandler(topic string, idx int) error {
	...
	if !(0 <= idx && idx < l) {
		return fmt.Errorf("index %d of handlers in topic %s invalid", idx, topic)
	}
